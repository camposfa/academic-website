...
```{r}
stickers_path <- here("assignments/week-09/data/PosidFazioCordes_StickerRich_Excel-Dataverse.xlsx")
```

Then read the spreadsheet:
...
We can rename single columns with the `dplyr` function `rename()`. Here's a basic example of how you use this function:

```{r}
#| eval: false
# Note: keep #| eval:false in this chunk!
...
5. Clean the names using janitor

```{r}
#| eval: false
stickers <- # Your chain of R commands starts here
```

<br>

## Messy Archeological Artifacts
...
```{r}
artifacts_path <- here("assignments/week-09/data/messy_ktc_data.xlsx")
```

<br>

### Problem 3: Reading in two easy sub-tables

The, for each sub-table (ceramic and lithic), provide the cell range to the `read_excel()` function.

First ceramic:

```{r}
#| eval: false
ceramic <- # Read the ceramic sub-table
...
```{r}
#| eval: false
lithic <- # Read the lithic sub-table 
...
### Problem 4: Reading in a more messy sub-table
...
#| eval: false
#| warning: false
mollusc <- # Read the mollusc sub-table using cell range
...
#| eval: false
#| warning: false
mollusc <- # Read the molluse sub-table following the instructions above
