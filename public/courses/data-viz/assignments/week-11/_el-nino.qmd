...
- Create a new variable (let's call it `oni_pos`) that will contain only the positive measured ONI values using, and if the oni value is less than 0, overwrite the value by setting it to 0. Then do the opposite to make the negative set. You can make these new variables with the `if_else()` inside `mutate()`. Use `?if_else()` to read about how to use the function.
